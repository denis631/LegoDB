...
let find_table db tbl_name =
  try
    ...
    |> List.find (fun tbl ->
           match tbl_name with TblName x -> Table.RegularTbl.name tbl = x)
  with _ ->
  ...
  let find_attr_for_tbl tbl =
    let filter_col (col, ty) =
      if col = attr_name then
        Some (Table.RegularTbl.Iu.make (Table.RegularTbl.name tbl) attr_name ty)
      else None
    in
    match List.filter_map filter_col @@ Table.RegularTbl.schema tbl with
    | [ x ] -> Some x
    | _ -> None
  in
  match

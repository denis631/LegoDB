...
module Expr = struct
  ...
  let rec ius expr =
    let rec ius_of_bool = function
      | And expr_lst -> List.unique @@ List.concat_map ius_of_bool expr_lst
      | Or expr_lst -> List.unique @@ List.concat_map ius_of_bool expr_lst
  ...
  let rec show expr =
    let rec show_bool = function
      | And expr_lst -> expr_lst |> List.map show_bool |> String.join " AND "
      | Or expr_lst -> expr_lst |> List.map show_bool |> String.join " OR "

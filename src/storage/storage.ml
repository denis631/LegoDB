...
module Value = struct
  ...
  let parse (t: ValueType.t) x = match t with 
    | Integer -> Integer (int_of_string x)
    | Numeric (len, precision) ->
      let trimmed_x = String.trim x in
      let is_neg = String.get trimmed_x 0 = '-' in
      let chars =
      ...
      let build_val (digits_seen, digits_seen_fraction, is_fraction, result) x =
        if x = '.' then
          if is_fraction then failwith "invalid number format: already in fraction"
          ...
          if cur >= zero && cur <= nine then
            if is_fraction then (digits_seen, digits_seen_fraction + 1, is_fraction, result * 10 + cur - zero)
            else (digits_seen + 1, digits_seen_fraction, is_fraction, result * 10 + cur - zero)
          else failwith "invalid number format: invalid character in numeric string"
      in
      let (digits_seen, digits_seen_fraction, _, numeric_val) = List.fold_left build_val (0, 0, false, 0) chars in
      if digits_seen > len || digits_seen_fraction > precision then failwith "invalid number format: loosing precision";
      Numeric ((len, precision), if is_neg then -numeric_val else numeric_val)
    (* TODO: add tests for parsing *)
    | Char _ -> Char x
    | VarChar _ -> VarChar x
    | Timestamp -> Timestamp (int_of_string x)
